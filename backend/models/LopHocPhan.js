...
module.exports = class LopHocPhan extends LOPHOCPHAN_MODEL {
    ...
    return new Promise(async (resolve) => {
      try {
        let data = await LOPHOCPHAN_MODEL.find({
          IDKhoaHoc: parseInt(IDKhoaHoc, 10),
        });
        if (!data)
          return resolve({
            error: true,
  ...
  static add({
    ...
    return new Promise(async (resolve) => {
      try {
        let lastLopHocPhan = await LOPHOCPHAN_MODEL.findOne().sort({
          IDLopHocPhan: -1,
        });
        let IDLopHocPhan = 1;
        let TrangThai = 1;
        ...
        let LopHocPhan = new LOPHOCPHAN_MODEL({
          IDKhoaHoc,
          IDLopHocPhan,
          MaLopHocPhan,
          TenLopHocPhan,
  ...
  static update({
    ...
    return new Promise(async (resolve) => {
      try {
        let checkID = await LOPHOCPHAN_MODEL.findOne({
          IDLopHocPhan: IDLopHocPhan,
        });
        if (!checkID)
          return resolve({
            error: true,
            message: "Không tìm thấy lớp học phần để sửa",
          });
        let updateID = await LOPHOCPHAN_MODEL.findOneAndUpdate(
          { IDLopHocPhan: IDLopHocPhan },
          {
            MaLopHocPhan,
            TenLopHocPhan,
  ...
  static delete(IDLopHocPhan) {
    return new Promise(async (resolve) => {
      try {
        ...
        let deleteLopHocPhan = await LOPHOCPHAN_MODEL.findOneAndDelete({
          IDLopHocPhan: IDLopHocPhan,
        });
