...
module.exports = class GiangVien extends GIANGVIEN_MODEL {
  ...
  static add({ HoTen, DiaChi, SoDienThoai, Email, MoTa, HinhAnh, GhiChu }) {
    return new Promise(async (resolve) => {
      try {
        ...
        let GiangVien = new GIANGVIEN_MODEL({
          IDGiangVien,
          TenTaiKhoan,
          HoTen,
          DiaChi,
        ...
        if (!saveGiangVien)
          return resolve({ error: true, message: "Không thể thêm giảng viên" });
        ...
        let datasample =
          ...
          "</body>";
        let NoiDung =
          "Thông tin tài khoản đăng nhập của bạn <br> Tên tài khoản: <strong>" +
          TenTaiKhoan +
          "</strong> <br> Mật khẩu: <strong>" +
          MatKhau +
          "</strong>";
  ...
  static update({
    ...
    return new Promise(async (resolve) => {
      try {
        let checkID = await GIANGVIEN_MODEL.findOne({
          IDGiangVien: IDGiangVien,
        });
        if (!checkID)
          return resolve({
            error: true,
            message: "Không tìm thấy giảng viên để sửa",
          });
        let updateID = await GIANGVIEN_MODEL.findOneAndUpdate(
          { IDGiangVien: IDGiangVien },
          { HoTen, DiaChi, SoDienThoai, Email, MoTa, HinhAnh, GhiChu },
          { new: true }
        );
  ...
  static updateThongTinCaNhan({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let updateID = await GIANGVIEN_MODEL.findOneAndUpdate(
          { TenTaiKhoan: TenTaiKhoan },
          { HoTen, DiaChi, SoDienThoai, Email, HinhAnh },
          { new: true }
        );
        resolve({ error: false, data: updateID });
      } catch (error) {
  ...
  static delete(IDGiangVien) {
    return new Promise(async (resolve) => {
      try {
        ...
        let deleteGiangVien = await GIANGVIEN_MODEL.findOneAndDelete({
          IDGiangVien: IDGiangVien,
        });
  ...
  static UpdateState(IDGiangVien, TrangThai) {
    return new Promise(async (resolve) => {
      try {
        ...
        await TAIKHOAN_MODELBD.findOneAndUpdate(
          { TenTaiKhoan: checkID.TenTaiKhoan },
          { TrangThai },
          { new: true }
        );
        resolve({ error: false, data: updateID });
