...
route.get('/getalllophocphandangkichhoat', async (req, res) => {
    let result = await LOPHOCPHAN_MODEL.getAllLopHocPhanDangKichHoat();
    return res.json({result});
});
...
route.put('/suatrangthai', async (req, res) => {
    let {IDLopHocPhan,TrangThai} = req.body;
    try {
        let result = await LOPHOCPHAN_MODEL.updateStatus({IDLopHocPhan,TrangThai});
        return res.json({"TrangThai":result})
    } catch (error) {
        return res.json({"TrangThai":result})
    }
